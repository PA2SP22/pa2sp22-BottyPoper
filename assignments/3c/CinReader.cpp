  ...
  if (range.length() > 0) {
    do {
      ...
      if (!testCharInput(input[0], range)) {
        if (charRangeErrors) cout << "[" << input[0] << "] " << charRangeMsg;
        input = "";
      } else
        valid = true;
    } while (!valid);
...
string CinReader::readString(bool allowEmpty, unsigned int limitTo) {
  ...
  if (limitTo > 0) {
    if (input.length() > limitTo) input = input.substr(0, limitTo);
  }
  return input;
}

void CinReader::showIntErrors(bool show) { intErrors = show; }

void CinReader::showCharRangeErrors(bool show) { charRangeErrors = show; }
...
void CinReader::setCharRangeMessage(string newMessage) {
  ...
  if (charRangeMsg.length() > 0 &&
      charRangeMsg[charRangeMsg.length() - 1] != ' ')
    charRangeMsg += ' ';
}

void CinReader::setIntMessage(string newMessage) {
  intMsg = newMessage;
  if (intMsg.length() > 0 && intMsg[intMsg.length() - 1] != ' ') intMsg += ' ';
}
...
string CinReader::stripCommas(string input) {
  ...
  for (unsigned int i = 0; i < input.length(); i++) {
    if (input[i] != ',') outs += input[i];
  }
  return outs;
}

bool CinReader::testIntInput(string input, string& msg, int lowerLimit,
                             int upperLimit) {
  int upperMarker = -1;
  int lowerMarker = -1;
  if (upperLimit == INT_MAX) upperMarker = 47;
  if (lowerLimit == INT_MIN) lowerMarker = 48;
...
bool CinReader::testBoolInput(string input) {
  if (input.length() > 0) {
    if (input.length() == 1) {
      ...
      if (ignoreCaseCompare(input, "TRUE") || ignoreCaseCompare(input, "FALSE"))
        return true;
      else
        return false;
    }

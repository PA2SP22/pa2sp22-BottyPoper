...
unsigned int TodoList::GetSize() { return list_size; }
unsigned int TodoList::GetCapacity() { return list_capacity; }
std::ostream &operator<<(std::ostream &original, TodoList &list_to_output) {
  for (unsigned int i = 0; i < list_to_output.GetSize(); i++) {
    original << i + 1 << ") " << list_to_output.bar[i]->description()
             << list_to_output.bar[i]->priority()
